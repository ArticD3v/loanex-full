...
const out = { fileExists: fs.existsSync(".env.vercel-pull"), totalKeys: Object.keys(env).length, mongoRelated: Object.keys(env).filter(k=>/mongo/i.test(k)), report: keys.map(k=>{ const v=env[k]||""; let kind="missing"; if(v){ if(/^mongodb/i.test(v)) kind="mongodb"; else if(/^postgres/i.test(v)) kind="postgres"; else if(/^https?:/i.test(v)) kind="http"; else kind="set"; } return {key:k, present:!!v, length:v.length, kind, looksRedacted:/redacted|sensitive/i.test(v)&&v.length<40}; }) };
